...
class KeyframeRepository(MongoBaseRepository[Keyframe]):
    ...
    async def get_keyframes_with_metadata_filter(
        ...
        """
        Get keyframes by keys with optional metadata and object filtering
        """
        query = {"key": {"$in": keys}}
        ...
        if metadata_filter or object_filter:
            ...
            if metadata_filter:
                ...
                if metadata_filter.get("min_length") is not None:
                    and_conditions.append(
                        {"length": {"$gte": metadata_filter["min_length"]}})
                        ...
                        and_conditions.append({
                            "$expr": {
                                "$gte": [
                                    {"$concat": [
                                        {"$arrayElemAt": [
                                            {"$split": ["$publish_date", "/"]}, 2]},
                                        {"$arrayElemAt": [
                                            {"$split": ["$publish_date", "/"]}, 1]},
                                        {"$arrayElemAt": [
                                            {"$split": ["$publish_date", "/"]}, 0]}
                                    ]},
                                    yyyymmdd_from
                                ]
                            }
            ...
            # Add object filters to the query
            if object_filter:
                target_objects = object_filter.get("objects", [])
                mode = object_filter.get("mode", "any")

                if target_objects:
                    # Normalize object names to lowercase for consistent matching
                    normalized_objects = [obj.lower().strip()
                                          for obj in target_objects if obj.strip()]

                    if normalized_objects:
                        if mode == "all":
                            # All objects must be present: use $all
                            and_conditions.append(
                                {"objects": {"$all": normalized_objects}})
                        else:  # mode == "any"
                            # At least one object must be present: use $in
                            and_conditions.append(
                                {"objects": {"$in": normalized_objects}})
